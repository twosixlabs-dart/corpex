...
class SearchControllerTest extends AnyFlatSpecLike with ScalatraSuite with Matchers with MockFactory with StandardCliConfig {
    ...
    addServlet( SearchController( ElasticsearchSearchService( config ), baseDependencies ), "/search/*" )

    "POST to /search" should "return 200 with valid json response for a valid request" in {
        ...
        post( "/search", body = """{"page":0,"page_size":10,"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
    ...
    "POST to /search/count" should "return 200 with valid json count response for a valid request" in {
        val mockServer : MockWebServer = new MockWebServer()
        val mockResponse = new MockResponse()
          .setResponseCode( 200 )
        ...
        post( "/search/count", body = """{"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
    ...
    "POST to /search/shave" should "return 400 when neither page_size field nor take query parameter have values" in {
        val mockServer : MockWebServer = new MockWebServer()
        ...
        post( "/search/shave", body = """{"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
            response.body should include("""Either the request body should include the 'page_size' field, or the query string should include the 'take' paramter""")
            status shouldBe 400
        }
    }

    "POST to /search/shave" should "return 200 and return a json list of doc ids of length equal to take query parameter if that is less than the number of results" in {
        val mockServer : MockWebServer = new MockWebServer()
        val mockResponse1 = new MockResponse()
          .setResponseCode( 200 )
          .setBody( mockEsScrollStartJson )
        ...
        post( "/search/shave?take=13", body = """{"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
    ...
    "POST to /search/shave" should "return 200 and return a json list of doc ids of length equal to take query parameter even if page_size query field is different" in {
        val mockServer : MockWebServer = new MockWebServer()
        val mockResponse1 = new MockResponse()
          .setResponseCode( 200 )
          .setBody( mockEsScrollStartJson )
        ...
        post( "/search/shave?take=13", body = """{"page_size":15,"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
            response.body shouldBe """["d5c413df85be46e6d27d0e7ba027858f","df7be300058ea8df8f4701670284c271","2b32f4a87d57ceb1a671b0d51f909453","5c43bb6c9df2f2238abfccd61506c985","2d79f269f0010ffbeaf26533184dfc14","0571770d3fc7573e515ac361e4b942c9","434e9d2ef2e95605e70d12e8a78c6aea","37ff939e43a2297b1ab9bb5b06d4e403","9243b122e7cc2e7447cdd87500d5f197","634f221da953e225ff9669bd620415d1","206ef8242affbb2f0b5b9ac86788b1c8","1d6edda33a134599943ef01361f15008","49b3e245cc36370aef84bb89b605b9c0"]"""
            status shouldBe 200
        }
    ...
    "POST to /search/shave" should "return 200 and return a json list of doc ids of length equal to page_size query field when take parameter is missing" in {
        val mockServer : MockWebServer = new MockWebServer()
        val mockResponse1 = new MockResponse()
          .setResponseCode( 200 )
          .setBody( mockEsScrollStartJson )
        ...
        post( "/search/shave", body = """{"page_size":11,"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
            response.body shouldBe """["d5c413df85be46e6d27d0e7ba027858f","df7be300058ea8df8f4701670284c271","2b32f4a87d57ceb1a671b0d51f909453","5c43bb6c9df2f2238abfccd61506c985","2d79f269f0010ffbeaf26533184dfc14","0571770d3fc7573e515ac361e4b942c9","434e9d2ef2e95605e70d12e8a78c6aea","37ff939e43a2297b1ab9bb5b06d4e403","9243b122e7cc2e7447cdd87500d5f197","634f221da953e225ff9669bd620415d1","206ef8242affbb2f0b5b9ac86788b1c8"]"""
            status shouldBe 200
        }
    }

    "POST to /search/shave" should "return 200 and return a json list of doc ids of length equal to number of results when take is larger than the number of results" in {
        val mockServer : MockWebServer = new MockWebServer()
        val mockResponse1 = new MockResponse()
          .setResponseCode( 200 )
          .setBody( mockEsScrollStartJson )
        ...
        post( "/search/shave?take=20", body = """{"page_size":11,"queries":[{"bool_type":"MUST","query_type":"TEXT","queried_fields":["cdr.extracted_text"],"query_string":"full text search"}]}""" ) {
            mockServer.shutdown()
            response.body shouldBe """["d5c413df85be46e6d27d0e7ba027858f","df7be300058ea8df8f4701670284c271","2b32f4a87d57ceb1a671b0d51f909453","5c43bb6c9df2f2238abfccd61506c985","2d79f269f0010ffbeaf26533184dfc14","0571770d3fc7573e515ac361e4b942c9","434e9d2ef2e95605e70d12e8a78c6aea","37ff939e43a2297b1ab9bb5b06d4e403","9243b122e7cc2e7447cdd87500d5f197","634f221da953e225ff9669bd620415d1","206ef8242affbb2f0b5b9ac86788b1c8","1d6edda33a134599943ef01361f15008","49b3e245cc36370aef84bb89b605b9c0","e7f51fc9edbe2c25bf502ddd636da770","84b35b53c4efe94966f42fd67a02c73c"]"""
            status shouldBe 200
        }
